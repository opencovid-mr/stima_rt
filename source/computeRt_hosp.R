...
file <- "https://raw.githubusercontent.com/ondata/covid19italia/master/webservices/iss_epicentro_dati/processing/ricoveri.csv"
# oppure
# File input archiviato da Andrea Mignone https://twitter.com/i_m_andrea
# file <- "https://raw.githubusercontent.com/floatingpurr/covid-19_sorveglianza_integrata_italia/main/data/latest/ricoveri.csv"

data <- read.csv(file)
data$RICOVERI <- suppressWarnings(as.numeric(data$RICOVERI)) #elimina i "<5"
data[is.na(data)] <- 0
...
for (i in toDo) {
  ...
  fileOut1 <- here::here(outputFolder, paste0("Rt_hosp_Ita_updated_", storico_date[i], ".csv"))
  write.csv(output, fileOut1, row.names = F)
  
  # Salva risultati piu' recenti in file a parte
  if(i == length(storico_date)){
    write.csv(output, here::here(outputFolder, "Rt_hosp_Ita_updated_latest.csv"), row.names = F)
  }
...
png(filename = here::here(outputFolder, "Rt_hosp_Ita_updated_latest.png"), width = 465, height = 225, units='mm', res = 300)

  ggplot(outputCons, aes(x = Data, y = R.medio)) +
  geom_smooth(aes(ymin = R.lowerCI, ymax = R.upperCI), stat = "identity") +
  geom_hline(yintercept = 1) +
  labs(title = paste0("Rt_hosp Italia fino al ", tail(outputCons$Data,1), 
