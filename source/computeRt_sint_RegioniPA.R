...
tipoDati <- "sintomatici" #da https://covid19.infn.it/iss/

# Giorni che si considerano NON consolidati 
int_consolidamento <- 14 # valore basato sui dati nazionali, ma potrebbe differire per Regione/PA

# Parametri ----
# Cambiare con nome cartella desiderata
outputFolder <- "output//Rt_sint_regionali"
...
intervallo.seriale <-
  dgamma(0:N, shape = shape.stimato, rate = rate.stimato)
...
base_path1 <- "https://covid19.infn.it/iss/csv_part/iss_80plus_"
base_path2 <- "_sintomatici.csv"

regioniPA <- c("abruzzo", "basilicata", "calabria", "campania", "emilia_romagna", "friuli_venezia_giulia",
               "lazio", "liguria", "lombardia", "marche", "molise", "piemonte", "puglia", "sardegna",
               "sicilia", "toscana", "umbria", "valle_daosta", "veneto", "pa_bolzano", "pa_trento", "italia")

for (tipo in tipoDati) {
  for (i in 1:length(regioniPA)) {
    
    data <- read.csv(paste0(base_path1, regioniPA[i], base_path2))
    data$data <- as.Date(data$data)
    data <- data[order(data$data),]
    data[is.na(data)] <- 0
    data$I <- data[,2] + data[,3]

    curva.epidemica <- data[, c("data", "I")]
    curva.epidemica[is.na(curva.epidemica)] <- 0
    colnames(curva.epidemica) <- c("Date", "I")
    
    updateData <- max(curva.epidemica$Date)
    ...
    p <- ggplot(outputCons, aes(x = Data, y = R.medio)) +
      geom_smooth(aes(ymin = R.lowerCI, ymax = R.upperCI), stat = "identity") +
      geom_hline(yintercept = 1) +
      labs(title = paste0("Rt_sintomi ",  toupper(regioniPA[i]), " fino al ", tail(outputCons$Data,1), 
                          " aggiornato al ", tail(output$Data,1), " con 95%CrI"), 
           x = "Data", y = "Rt_sint",
           subtitle = paste0("ATTENZIONE: dati ultimi ", int_consolidamento, " giorni esclusi in quanto non consolidati (basato sul dato nazionale)."),
           caption = paste0("Rt ottenuto con EpiEstim (tw=7gg, shape=", shape.stimato, " rate=", rate.stimato, ") da dati Istituto Superiore Sanità via INFN")) +
    ...
    ggsave(filename = here::here(outputFolder, paste0("Rt_sint_", regioniPA[i], "_updated_latest.png")), p,
